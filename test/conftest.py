...
def tresults():
    results = [{'name': 'test.{}'.format(r), 'green': r, 'blue': r, 'red': r}
...
def tdirectory():
    ...
    for value in [0, 127, 255]:
        im = Image.new("RGB", (200, 200), "rgb({0}, {0}, {0})".format(value))
        im.save(os.path.join(t_directory.name, '{}.png'.format(value)),
                format="png")
    return(t_directory)
...
def tdirectories():
    t_directory = tempfile.TemporaryDirectory(prefix='{}.'.format(127))
    for value in [0, 127, 255]:
        subpath = tempfile.mkdtemp(prefix='{}.'.format(value),
                                   dir=t_directory.name)
        for num in range(3):
            im = Image.new("RGB", (200, 200), "rgb({0}, {0}, {0})"
                           .format(value))
            im.save(os.path.join(subpath, '{}.{}.png'
                                 .format(value, num)), format="png")
    return(t_directory)
