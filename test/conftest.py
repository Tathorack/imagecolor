    ...
    def make_test_directory(values, size=(200, 200)):
        ...
        for value in values:
            ...
            image.save(os.path.join(
                test_directory.name, '{}.png'.format(value)), format="png")
    ...
    def make_nested_test_directories(values, size=(200, 200), image_count=3):
        ...
        for value in values:
            subpath = tempfile.mkdtemp(
                prefix='{}.'.format(value),
                dir=test_root_directory.name)
            for count in range(image_count):
                image = Image.new(
                    "RGB", size,
                    "rgb({0}, {0}, {0})".format(value))
                image.save(
...
def test_results_creator():
    def make_test_results(values):
        results = [{'name': 'test.{}'.format(value), 'green': value,
                    'blue': value, 'red': value} for value in values]
        return results
